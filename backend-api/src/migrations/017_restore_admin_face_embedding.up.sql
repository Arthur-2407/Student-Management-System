...
--   4. Set students.face_enrolled = TRUE for the admin.
-- =================================================================
...
BEGIN
  -- Get the internal ID of the admin student
  SELECT id INTO v_admin_id FROM students WHERE student_id = 'admin';
  
  IF v_admin_id IS NULL THEN
    RAISE NOTICE '[Migration 017] Admin user (student_id = admin) not found. Skipping recovery.';
    RETURN;
  END IF;

  -- Count currently active face embeddings for admin
  SELECT COUNT(*) INTO v_active_count FROM face_embeddings 
  WHERE student_id = v_admin_id AND is_active = TRUE;
  ...
  RAISE NOTICE '[Migration 017] Reactivated face embedding id=% for admin (student_id=1).', v_target_embedding_id;
  ...
  RAISE NOTICE '[Migration 017] Synced students.face_enrolled = TRUE for admin.';
END $$;
