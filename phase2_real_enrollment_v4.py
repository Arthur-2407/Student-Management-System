#!/usr/bin/env python3
"""
REAL Face Enrollment v4 - Creates employees then enrolls faces
"""
...
MAIN_DB_PORT = 5432
MAIN_DB_NAME = "attendance_system"
MAIN_DB_USER = "postgres"
MAIN_DB_PASSWORD = "securepassword123"
...
def create_test_employee(employee_id: str, name: str) -> int:
    """Create test employee in main database. Returns employee_id (integer)."""
    ...
    try:
        ...
        if result:
            print(f"      Already exists, ID={result[0]}")
            cursor.close()
            conn.close()
            return result[0]
        ...
        cursor.execute(
            """INSERT INTO employees 
               (employee_id, first_name, last_name, email, department, position, role, hire_date, is_active)
               VALUES (%s, %s, %s, %s, 'Test', 'Test User', 'employee', NOW(), TRUE)
               RETURNING id""",
            (employee_id, first_name, last_name, f"{employee_id}@test.local")
        )
...
def store_embedding_in_db(emp_db_id: int, embedding: List[float], confidence: float, model_version: str) -> bool:
    ...
    try:
        ...
        # Insert new (use employee_id as integer!)
        ...
        cursor.execute(
            """INSERT INTO face_embeddings 
               ...
               RETURNING id""",
            (emp_db_id, embedding_json, confidence, model_version)
        )
...
def main():
    print("\n" + "="*80)
    print("REAL FACE ENROLLMENT - Phase 2 Final")
    print("="*80)
    ...
    for emp in TEST_EMPLOYEES:
        ...
        try:
            ...
            emp_db_id = create_test_employee(employee_id, name)
            if emp_db_id is None:
                results.append({"employee_id": employee_id, "status": "FAILED", "error": "Employee creation failed"})
                continue
            ...
            if not store_embedding_in_db(emp_db_id, embedding, 0.85, "2.0-facenet-vggface2"):
            ...
            print(f"\n[EXCEPTION] {str(e)[:100]}")
            results.append({"employee_id": employee_id, "status": "FAILED", "error": str(e)[:80]})
    ...
    if failed:
        print(f"\nFailed: {len(failed)}")
        for r in failed:
            print(f"  ✗ {r['employee_id']}: {r.get('error', 'Unknown')}")
