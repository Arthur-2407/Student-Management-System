#!/usr/bin/env python3
"""
REAL Face Enrollment v6 - Direct SQL file execution
"""
...
def create_test_employee_sql(employee_id: str, name: str) -> int:
    """Create test employee using SQL file."""
    ...
    try:
        ...
        sql_lines = [
            ...
            f"INSERT INTO employees (employee_id, first_name, last_name, email, department, position, role, hire_date, is_active)",
            f"  VALUES ('{employee_id}', '{first_name}', '{last_name}', '{employee_id}@test.local', 'Test', 'Test User', 'employee', NOW(), TRUE);",
            f"SELECT id FROM employees WHERE employee_id = '{employee_id}';"
        ]
        ...
        result = subprocess.run(
            ["docker", "exec", "-i", "attendance-db", "psql", "-U", "postgres", "-d", "attendance_system"],
        ...
        print(f"      ERROR: Could not parse employee ID")
        return None
...
def store_embedding_sql(emp_db_id: int, embedding: List[float], confidence: float) -> bool:
    """Store embedding using SQL file."""
    try:
        emb_json = json.dumps(embedding)
        # Escape for SQL
        emb_json_escaped = emb_json.replace("'", "''")
        
        sql_lines = [
            f"UPDATE face_embeddings SET is_active = FALSE WHERE employee_id = {emp_db_id} AND is_active = TRUE;",
            f"INSERT INTO face_embeddings (employee_id, embedding_vector, confidence_score, model_version, is_active, created_at)",
            f"  VALUES ({emp_db_id}, '{emb_json_escaped}', {confidence}, '2.0-facenet-vggface2', TRUE, NOW());",
            f"SELECT id FROM face_embeddings WHERE employee_id = {emp_db_id} ORDER BY created_at DESC LIMIT 1;"
        ]
        ...
        # Execute
        result = subprocess.run(
            ["docker", "exec", "-i", "attendance-face-db", "psql", "-U", "face_admin", "-d", "attendance_face_system"],
            stdin=open(sql_file, 'r'),
            capture_output=True,
            text=True,
            timeout=10
        ...
        for line in lines:
            if line.isdigit():
                embedding_id = int(line)
                emb_np = np.array(embedding, dtype=np.float32)
                norm = np.linalg.norm(emb_np)
                print(f"      ID={embedding_id}, norm={norm:.4f}")
                return True
        
        print(f"      ERROR: Could not parse embedding ID")
        return False
...
def main():
    print("\n" + "="*80)
    print("REAL FACE ENROLLMENT - Phase 2 (SQL Files)")
    print("="*80)
    ...
    for emp in TEST_EMPLOYEES:
        ...
        try:
            ...
            emp_db_id = create_test_employee_sql(employee_id, name)
            ...
            if not enroll_response.get("success"):
                print(f"    ERROR: {enroll_response.get('error')}")
                results.append({"employee_id": employee_id, "status": "FAILED"})
                continue
            ...
            if not store_embedding_sql(emp_db_id, embedding, 0.85):
                results.append({"employee_id": employee_id, "status": "FAILED"})
                continue
    ...
    for r in successful:
        print(f"  ✓ {r['employee_id']} (DB ID={r['db_id']}): {r['embedding_dim']}D")
    
    if len(successful) > 0:
        print(f"\n✓ PHASE 2 COMPLETE - REAL embeddings enrolled")
