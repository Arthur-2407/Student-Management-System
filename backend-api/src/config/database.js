...
async function seedFaceDatabase() {
  ...
  try {
    ...
    if (adminRes.rows.length > 0) {
      ...
      await query("UPDATE employees SET face_enrolled = TRUE, face_enrolled_at = NOW(), face_enrolled_by = $1 WHERE id = $1", [adminId]);
    }

    if (supervisorRes.rows.length > 0) {
      const supervisorId = supervisorRes.rows[0].id;
      const adminId = adminRes.rows.length > 0 ? adminRes.rows[0].id : supervisorId;
      await faceQuery(
        `INSERT INTO face_embeddings (employee_id, embedding_vector, embedding_version, confidence_score, enrolled_by)
         VALUES ($1, $2, '1.0', 1.0, $3) ON CONFLICT DO NOTHING`,
        [supervisorId, v_vector, adminId]
      );
      await query("UPDATE employees SET face_enrolled = TRUE, face_enrolled_at = NOW(), face_enrolled_by = $2 WHERE id = $1", [supervisorId, adminId]);
    }
...
async function migrateToUserImages(client) {
  try {
    const activeEmbeddings = await client.query(
      'SELECT employee_id, embedding_vector, enrollment_date FROM face_embeddings WHERE is_active = TRUE'
    );
    for (const row of activeEmbeddings.rows) {
      // Fetch user details from main database
      const empRes = await query('SELECT first_name, last_name FROM employees WHERE id = $1', [row.employee_id]);
      if (empRes.rows.length > 0) {
        ...
        const imgExists = await client.query('SELECT image_id FROM user_images WHERE user_id = $1 LIMIT 1', [row.employee_id]);
        if (imgExists.rows.length === 0) {
          await client.query(
            ...
            [row.employee_id, JSON.stringify(emb), row.enrollment_date]
          );
        }
      }
    }
