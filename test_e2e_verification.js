...
function runQuery(sql) {
  ...
  const cmd = `docker exec student-db psql -U postgres -d student_system -c "${sanitizedSql}"`;
  try {
...
function runFaceQuery(sql) {
  const sanitizedSql = sql.replace(/"/g, '\\"');
  const cmd = `docker exec student-face-db psql -U face_admin -d student_face_system -c "${sanitizedSql}"`;
  try {
    return execSync(cmd, { encoding: 'utf8' }).trim();
  } catch (err) {
...
async function runAllTests() {
  ...
  try {
    execSync('docker exec student-db pg_dump -U postgres -d student_system -F c -b -f /tmp/student_system_backup.dump', { stdio: 'inherit' });
    execSync('docker exec student-face-db pg_dump -U face_admin -d student_face_system -F c -b -f /tmp/student_face_system_backup.dump', { stdio: 'inherit' });
  ...
  try {
    console.log('Restarting student-backend container...');
    execSync('docker restart student-backend', { stdio: 'inherit' });
    console.log('Restarting student-nginx container to refresh upstream DNS...');
    execSync('docker restart student-nginx', { stdio: 'inherit' });
  ...
  if (overallPass) {
    ...
    try {
      ...
      execSync('docker exec student-db psql -U postgres -d student_system -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO postgres; GRANT ALL ON SCHEMA public TO public;"', { stdio: 'inherit' });
      // Restore student_system
      execSync('docker exec student-db pg_restore -U postgres -d student_system -v /tmp/student_system_backup.dump', { stdio: 'inherit' });

      // Recreate public schema for student_face_system
      execSync('docker exec student-face-db psql -U face_admin -d student_face_system -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO face_admin; GRANT ALL ON SCHEMA public TO public;"', { stdio: 'inherit' });
      // Restore student_face_system
      execSync('docker exec student-face-db pg_restore -U face_admin -d student_face_system -v /tmp/student_face_system_backup.dump', { stdio: 'inherit' });
    ...
    try {
      execSync('docker exec student-db rm -f /tmp/student_system_backup.dump', { stdio: 'ignore' });
      execSync('docker exec student-face-db rm -f /tmp/student_face_system_backup.dump', { stdio: 'ignore' });
    } catch (e) {}
