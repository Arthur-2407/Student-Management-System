...
-- Purpose: Add approval workflow tables for face modifications and seed default teacher + default embeddings
-- Date: 2026-06-14
...
CREATE TABLE IF NOT EXISTS face_change_requests (
  ...
  requested_by  INTEGER NOT NULL REFERENCES students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS face_approval_history (
  ...
  actioned_by     INTEGER NOT NULL REFERENCES students(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS face_audit_logs (
  ...
  performed_by          INTEGER NOT NULL REFERENCES students(id) ON DELETE RESTRICT,
...
-- ============================================================================
-- 5. SEED DEFAULT TEACHER ACCOUNT
-- ============================================================================
INSERT INTO students (
  ...
  'Teacher',
  'User',
  'teacher@attendance-system.local',
  '+1-555-0200',
  'Management',
  'Team Teacher',
  'teacher',
...
BEGIN
  ...
  -- Get student IDs
  SELECT id INTO v_admin_id FROM students WHERE student_id = 'admin';
  SELECT id INTO v_teacher_id FROM students WHERE student_id = 'teacher';

  -- Register admin face embedding if admin exists
  IF v_admin_id IS NOT NULL THEN
    -- Deactivate old active embeddings for admin
    UPDATE face_embeddings SET is_active = FALSE WHERE student_id = v_admin_id;
  ...
  -- Register teacher face embedding if teacher exists
  IF v_teacher_id IS NOT NULL THEN
    UPDATE face_embeddings SET is_active = FALSE WHERE student_id = v_teacher_id;

    INSERT INTO face_embeddings (
      student_id, embedding_vector, embedding_version, confidence_score, enrolled_by
    ) VALUES (
      v_teacher_id, v_vector, '1.0', 1.0, v_admin_id
    );
