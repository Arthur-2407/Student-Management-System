...
describe('ImpossibleTravelDetector', () => {
  ...
  test('first login is never a threat', () => {
    const detector = new ImpossibleTravelDetector();
    const result = detector.check('user1', { lat: 40.7128, lng: -74.0060, timestamp: Date.now() });
    expect(result.isThreat).toBe(false);
  });

  test('nearby login is not a threat', () => {
    const detector = new ImpossibleTravelDetector();
    const now = Date.now();
    detector.check('user2', { lat: 40.7128, lng: -74.0060, timestamp: now });
    // 10km away, 1 hour later
    const result = detector.check('user2', { lat: 40.8, lng: -74.0, timestamp: now + 3600000 });
    expect(result.isThreat).toBe(false);
  });

  test('impossible travel triggers a threat', () => {
    const detector = new ImpossibleTravelDetector();
    const now = Date.now();
    // New York
    detector.check('user3', { lat: 40.7128, lng: -74.0060, timestamp: now });
    // Tokyo, 5 minutes later (impossible)
    const result = detector.check('user3', { lat: 35.6762, lng: 139.6503, timestamp: now + 300000 });
    expect(result.isThreat).toBe(true);
    expect(result.details.severity).toBeDefined();
    expect(result.details.distanceKm).toBeGreaterThan(1000);
  });

  test('no location returns no threat', () => {
    const detector = new ImpossibleTravelDetector();
    const result = detector.check('user4', null);
    expect(result.isThreat).toBe(false);
  });

  test('getStats returns tracked users and alerts', () => {
    const detector = new ImpossibleTravelDetector();
    detector.check('user5', { lat: 40.7, lng: -74.0, timestamp: Date.now() });
    const stats = detector.getStats();
    expect(stats.trackedUsers).toBe(1);
    expect(stats.totalAlerts).toBe(0);
  });

  test('clearUser removes tracking data', () => {
    const detector = new ImpossibleTravelDetector();
    detector.check('user6', { lat: 40.7, lng: -74.0, timestamp: Date.now() });
    detector.clearUser('user6');
    expect(detector.getStats().trackedUsers).toBe(0);
  });
});
